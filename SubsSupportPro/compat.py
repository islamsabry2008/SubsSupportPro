...
def LanguageEntryComponent(file, name, index):
    ...
    plugin_countries = os.path.join(os.path.dirname(__file__), 'img', 'countries')

    def add_country_candidates(items, value):
        if value is None:
            return
        value = str(value)
        if not value:
    ...
    country_codes = []
    add_country_candidates(country_codes, file)
    add_country_candidates(country_codes, index)
    if isinstance(index, str) and '_' in index:
        add_country_candidates(country_codes, index.split('_', 1)[0])
        add_country_candidates(country_codes, index.split('_', 1)[1])

    candidates = [
    ]
    for code in country_codes:
        candidates.append(os.path.join(plugin_countries, code + '.png'))
    candidates.append(os.path.join(plugin_countries, 'missing.png'))
    for code in country_codes:
        candidates.append(resolveFilename(SCOPE_CURRENT_SKIN, 'countries/' + code + '.png'))
    candidates.append(resolveFilename(SCOPE_CURRENT_SKIN, 'countries/missing.png'))

    png = None
    for candidate in candidates:
        try:
            if candidate and fileExists(candidate):
                png = LoadPixmap(candidate)
            elif candidate and candidate.startswith('/'):
                png = LoadPixmap(candidate)
            if png is not None:
                break
        except Exception:
