...
try:
    ...
    def pbkdf2_hmac(name, password, salt, iterations, dklen=None):
        ...
        while len(b"".join(blocks)) < dklen:
            u = hmac.new(password, salt + pack(">I", block), name).digest()
            out = bytearray(u)
            for _ in range(iterations - 1):
                u = hmac.new(password, u, name).digest()
                out = bytearray([a ^ b for a, b in zip(out, bytearray(u))])
    ...
    def _make_symlink(self, info, dstfn, pwd, set_attrs):
        ...
        if info.host_os == RAR_OS_UNIX:
            ...
            if redir_type == RAR5_XREDIR_WINDOWS_JUNCTION:
                warnings.warn('Windows junction not supported - {}'.format(info.filename), UnsupportedWarning)
                return None
            target_is_directory = (redir_type & RAR5_XREDIR_ISDIR) > 0
        else:
            warnings.warn('Unsupported link type - {}'.format(info.filename), UnsupportedWarning)
            return None
...
class PipeReader(RarExtFile):
    ...
    def _open_extfile(self, parser, inf):
        RarExtFile._open_extfile(self, parser, inf)
...
def load_windowstime(buf, pos):
    ...
    dt = datetime.utcfromtimestamp(secs - unix_epoch)
...
class nsdatetime(datetime):
    ...
    def __new__(cls, year, month, day, hour=0, minute=0, second=0,
        ...
        self = datetime.__new__(cls, year, month, day, hour, minute, second, usec, tzinfo)
    ...
    def isoformat(self, sep="T", timespec="auto"):
        ...
        """
        if timespec == "auto":
            pre, post = datetime.isoformat(self, sep).split(".", 1)
    ...
    def astimezone(self, tz=None):
        ...
        """
        tmp = datetime.astimezone(self, tz)
        return self.__class__(tmp.year, tmp.month, tmp.day, tmp.hour, tmp.minute, tmp.second,
                              nanosecond=self.nanosecond, tzinfo=tmp.tzinfo, fold=getattr(tmp, "fold", 0))
    ...
    def replace(self, year=None, month=None, day=None, hour=None, minute=None, second=None,
        ...
        return self.__class__(
            ...
            fold=getattr(self, "fold", 0) if fold is None else fold)

    def __hash__(self):
        return hash((datetime.__hash__(self), self.nanosecond)) if self.nanosecond else datetime.__hash__(self)

    def __eq__(self, other):
        return datetime.__eq__(self, other) and self.nanosecond == (
            other.nanosecond if isinstance(other, nsdatetime) else other.microsecond * 1000)

    def __gt__(self, other):
        return datetime.__gt__(self, other) or (datetime.__eq__(self, other) and self.nanosecond > (
            other.nanosecond if isinstance(other, nsdatetime) else other.microsecond * 1000))
...
def to_nsdatetime(dt, nsec):
    ...
    return nsdatetime(dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second,
                      tzinfo=dt.tzinfo, fold=getattr(dt, "fold", 0), nanosecond=nsec)
...
def to_nsecs(dt):
    ...
    """
    import calendar
    secs = int(calendar.timegm(dt.utctimetuple()))
