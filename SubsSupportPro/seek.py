...
try:
    from .seekers import (
        SubtitlesDownloadError, SubtitlesSearchError, SubtitlesErrors,
        SubtitlesmoraSeeker, NovalermoraSeeker, OpenSubtitles2Seeker,
        Subf2mSeeker, LocalDriveSeeker, SubdlSeeker,
        SubsourceSeeker, WyzieSeeker
    )
...
class SubsSeeker(object):
    ...
    def getSubtitles(self, providers, updateCB=None, title=None, filepath=None, langs=None, year=None, tvshow=None, season=None, episode=None, timeout=10):
        ...
        if len(providers) == 1:
            ...
            if provider is None:
                self.log.debug("provider '%s' not found, skipping...", providers[0])
            ...
            for provider in providers:
                ...
                if provider is None:
                    self.log.debug("provider not found, skipping...")
                    continue
                if provider.error is not None:
                    self.log.debug("provider '%s' has 'error' flag set, skipping...", provider)
                else:
    ...
    def _searchSubtitles(self, lock, subtitlesDict, updateCB, seeker, title, filepath, langs, season, episode, tvshow, year):
        try:
            ...
            traceback.print_exc()
            with lock:
                error_message = toUnicode(e)
                subtitlesDict[seeker.id] = {'message': error_message, 'status': False, 'list': []}
                if updateCB is not None:
                    updateCB(seeker.id, False, error_message)
        else:
            with lock:
