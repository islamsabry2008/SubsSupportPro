...
from .e2_utils import (
    messageCB, E2SettingsProvider, SubsProLanguageSelection, unrar, ConfigFinalText,
    Captcha, DelayMessageBox, MyConfigList, getFps, fps_float, getFonts,
    BaseProMenuScreen, isFullHD, isHD, getDesktopSize
)
...
def _showThemeRestartPluginMessage(session):
    ...
    message = _("Theme saved.\n\nPlease close and reopen SubsSupportPro to apply the selected theme.")
    ...
    def _showMessage():
        ...
        try:
            ...
            try:
                Notifications.AddNotification(MessageBox, message, MessageBox.TYPE_INFO, timeout=8)
            except Exception:
                pass
...
def _subpro_country_png(country, cache=None):
    countries_path = os.path.join(os.path.dirname(__file__), 'img', 'countries')
...
DEFAULT_TMDB_DATA_PATH = "/etc/enigma2/subssupportpro"
...
def _get_safe_tmdb_data_path(update_config=True):
    ...
    selected_path = _ensure_directory(configured_path, allow_create_parent=False)
    if selected_path is None:
        selected_path = _ensure_directory(DEFAULT_TMDB_DATA_PATH, allow_create_parent=True)

    if selected_path is None:
        selected_path = _ensure_directory("/var/volatile/tmp/subssupportpro", allow_create_parent=True)

    if selected_path is None:
        selected_path = "/tmp/subssupportpro"
        if not os.path.isdir(selected_path):
            os.makedirs(selected_path)

    if update_config:
        try:
            current_path = _normalize_config_path(config.plugins.subtitlesSupportPro.settingsTMDBDataPath.value)
...
def debug(text, *args):
    if DEBUG:
        if len(args) == 1 and isinstance(args[0], tuple):
            ...
            text = text % (args)
        print("[SubsSupportPro]", toString('utf-8'))
...
# Theme color roles are based on the current default subskins.xml palette.
# Changing a theme replaces these exact base colors in every loaded subskin.
_THEME_COLOR_ROLES = (
    ("#8e5245", "header"),
    ("#27408b", "border"),
    ("#061425", "background"),
    ...
    ("#2c525b", "selection"),
    ("#5d37d9", "accent"),
    # Compatibility aliases used by some embedded/dynamic skins.
    ("#45818e", "selection"),
    ("#081a31", "panel"),
    ("#6854a8", "accent"),
)

SUBSSUPPORTPRO_THEMES = {
    "ember": {
        "name": _("Ember"),
        "header": "#8e5245",
        "border": "#27408b",
...
def _get_subssupportpro_theme_key():
    try:
        ...
        if theme_key in SUBSSUPPORTPRO_THEMES:
            return theme_key
    except Exception:
        pass
    return "ember"
...
def _apply_subssupportpro_theme(skin_text):
    if not skin_text:
        return skin_text
    theme = SUBSSUPPORTPRO_THEMES.get(_get_subssupportpro_theme_key(), SUBSSUPPORTPRO_THEMES["ember"])
...
def _set_subssupportpro_screen_skin(screen, skin_base, force_zposition=None):
    ...
    """
    try:
        suffix = "_fhd" if isFullHD() else "_hd"
    except Exception:
        suffix = "_fhd"
    try:
        skin_text = load_subskin(skin_base + suffix, getattr(screen, "skin", None))
        if skin_text and force_zposition is not None:
            try:
                skin_text = re.sub(r'zPosition="[^"]+"', 'zPosition="%s"' % force_zposition, skin_text, count=1)
            except Exception:
                pass
        if skin_text:
        ...
        try:
            print("[SubsSupportPro] Theme skin apply failed for %s: %s" % (skin_base, e))
        except Exception:
            pass
...
def reloadThemedSubskins():
    ...
    suffix = "_fhd" if isFullHD() else "_hd"
    skin_classes = {
        "SubsProMenu": "SubsProMenu",
        "TMDBScraperProScreen": "TMDBScraperProScreen",
        "MovieDetailsProScreen": "MovieDetailsProScreen",
        "SubsProChooser": "SubsProChooser",
        "SubsProDownloadedSelection": "SubsProDownloadedSelection",
        "SubsProEmbeddedSelection": "SubsProEmbeddedSelection",
        "SubsProSearchDownloadOptions": "SubsProSearchDownloadOptions",
        "SubsProSearchContextMenu": "SubsProSearchContextMenu",
        "SubsSupportProLogScreen": "SubsSupportProLogScreen",
        "SubsProSearch": "SubsProSearch",
        "SubsProSearchSettings": "SubsProSearchSettings",
    }
    for cls_name, skin_base in skin_classes.items():
        cls = globals().get(cls_name)
        if cls is not None:
            try:
                cls.skin = load_subskin(skin_base + suffix, getattr(cls, "skin", None))
            except Exception:
                pass
    try:
        SubsProDownloadedSelection.InfoProScreen.skin = load_subskin("InfoProScreen" + suffix, SubsProDownloadedSelection.InfoProScreen.skin)
    except Exception:
        pass
...
print('[SubsSupportPro] looking for custom colors in', COLORFILE)
try:
    with open(COLORFILE, 'r') as f:
        for line in f:
            ...
            if color is not None:
                ...
                print('[SubsSupportPro] adding custom color', alias)
                colorChoiceList.append((hex_color, alias))
except IOError as e:
    print('[SubsSupportPro] error while loading custom colors', str(e))
...
def initGeneralProSettings(configsubsection):
    ...
    configsubsection.settingsBackupPath = ConfigText(default="/etc/enigma2/subssupportpro", fixed_size=False, visible_width=40)
    configsubsection.settingsTMDBDataPath = ConfigText(default="/etc/enigma2/subssupportpro", fixed_size=False, visible_width=40)
    configsubsection.theme = ConfigSelection(default="ember", choices=SUBSSUPPORTPRO_THEME_CHOICES)
...
def initSearchSettings(configsubsection):
    ...
    configsubsection.downloadHistory.path = ConfigDirectory(default=eEnv.resolve("$localstatedir/lib/subssupportpro"), visible_width=30)
...
def initSubsProSettings(configSubsection=None):
    global GLOBAL_CONFIG_INIT
    if configSubsection:
        print('[SubsSupportPro] using provided ConfigSubsection to store config')
        subtitles_settings = configSubsection
    elif 'PLUGIN_NAME' in globals():
        print('[SubsSupportPro] using config.plugins.%s.%s to store config' % (PLUGIN_NAME, 'subtitles'))
        ...
        print("[SubsSupportPro] using global config (already initialized)")
        return config.plugins.subtitlesSupportPro
    else:
        print("[SubsSupportPro] using global config")
...
class SubsProSupportStatus(object):
...
class SubsProSupportEmbedded(object):
    ...
    def __init__(self, embeddedSupport, preferEmbedded):
        ...
        self.subtitle_window = self.session.instantiateDialog(SubsEmbeddedScreen, self.subsProSettings.embedded)
    ...
    def enableSubtitle(self, selectedSubtitle):
        print('[SubsProSupportEmbedded] enableSubtitle', selectedSubtitle)
        subtitle = self.getCurrentServiceSubtitle()
        ...
        if subtitle and self.selected_subtitle:
            ...
            print('[SubsProSupportEmbedded] enable embedded subtitles')
        else:
            print('[SubsProSupportEmbedded] disable embedded subtitles')
            if subtitle:
    ...
    def resetEmbeddedSubs(self, reloadScreen=False):
        ...
        if reloadScreen:
            print('[SubsProSupportEmbedded] reloading embedded screen')
            subtitle = self.getCurrentServiceSubtitle()
            if subtitle:
                subtitle.disableSubtitles(self.subtitle_window.instance)
...
class SubsProSupport(SubsProSupportEmbedded):
    ...
    def __init__(self, session=None, subsPath=None, defaultPath=None, forceDefaultPath=False, autoLoad=True,
        ...
        self.subsProSettings = initSubsProSettings(configEntry)
        SubsProSupportEmbedded.__init__(self, embeddedSupport, preferEmbedded)
        self.__subsScreen = self.session.instantiateDialog(SubsScreen, self.subsProSettings.external)
        self.__subsScreen.hide()
        self.__subsEngine = SubsEngine(self.session, self.subsProSettings.engine, self.__subsScreen)
        self.__subsLoader = SubsLoader(PARSERS, ALL_LANGUAGES_ENCODINGS + ENCODINGS[self.subsProSettings.encodingsGroup.getValue()])
        ...
        self.__startDelay = int(self.subsProSettings.engine.expert.startDelay.value)
    ...
    def subsMenu(self):
        if not self.__working and not (self.__subclassOfScreen and not self.__isServiceSet):
            ...
            if self.subsProSettings.pauseVideoOnSubtitlesMenu.value:
                print('[SubsSupportPro] stopVideoOnSubtitlesMenu: True')
                if isinstance(self, InfoBarSeek):
                    if self.seekstate == InfoBarSeek.SEEK_STATE_PLAY:
                        print('[SubsSupportPro] pausing video')
                        self.setSeekState(InfoBarSeek.SEEK_STATE_PAUSE)
                    else:
                        print('[SubsSupportPro] video is already paused')
                        self.__alreadyPausedVideo = True
                else:
                    print('[SubsSupportPro] not subclass of InfobarSeek')
            self.session.openWithCallback(self.__subsMenuCB, SubsProMenu, self,
    ...
    def resetSubs(self, resetEnc=True, resetEngine=True, newSubsScreen=False, newService=True):
        ...
        if resetEnc:
            self.__subsEnc = None
        self.__subsLoader.change_encodings(ALL_LANGUAGES_ENCODINGS + ENCODINGS[self.subsProSettings.encodingsGroup.getValue()])
        self.__subsEngine.pause()
        if resetEngine:
    ...
    def __resetSubsScreen(self, newSubsScreen=False):
        ...
        if newSubsScreen:
            ...
            self.__subsScreen = self.session.instantiateDialog(self._getSubsScreenCls(), self.subsProSettings.external)
    ...
    def exitSubs(self):
        ...
        print('[SubsSupportPro] closing subtitleDisplay')
    ...
    def __subsMenuCB(self, subsPath, subsEmbedded, settingsChanged, changeEncoding,
        ...
        if self.embeddedEnabled and self.embeddedSupport and not subsEmbedded and not turnOff and not subsPath:
            ...
            print('[SubsSupportPro] loading embedded subtitles')
            if self.__loaded:
                self.resetSubs()
            self.__subsScreen.hide()
            ...
            if self.__subsPath == subsPath:
                if not settingsChanged and not ((changeEncoding or changedEncodingGroup) or newScreen or forceReload):
                    print('[SubsSupportPro] no changes made')
        ...
        if not self.__alreadyPausedVideo and isinstance(self, InfoBarSeek):
            print('[SubsSupportPro] unpausing video')
    ...
    def __serviceStarted(self):
        print('[SubsSupportPro] Service Started')
    ...
    def __serviceStopped(self):
        print('[SubsSupportPro] Service Stopped')
        self.__starTimer.stop()
        self.resetSubs(True)
        self.__isServiceSet = False
...
class SubtitlesWidget(GUIComponent):
    ...
    def calcWidgetHeight(self):
        backgroundType = config.plugins.subtitlesSupportPro.external.background.type.value  # Get background type
        
        if backgroundType == "fixed":
            heightFactor = int(config.plugins.subtitlesSupportPro.external.background.height.value)  # Use user setting
        else:
    ...
    def setSubtitle(self, sub):
        ...
        if translate_cfg is not None and _get_translate_mode(self.externalSettings) == "line":
            try:
                ...
                print('[SubsSupportPro] line translation failed:', e)
...
class SubsProMenu(Screen):
    if isFullHD:
        skin = load_subskin("SubsProMenu_fhd")
    else:
        skin = load_subskin("SubsProMenu_hd")
    ...
    def __init__(self, session, infobar, subfile=None, subdir=None, encoding=None, embeddedSupport=False, embeddedEnabled=False, searchSupport=False):
        _set_subssupportpro_screen_skin(self, "SubsProMenu")
        Screen.__init__(self, session)
    ...
    def initTitle(self):
        self.setTitle("SubsSupportPro %s" % __version__)
    ...
    def ok(self):
        ...
        if mode == 'choose':
            self.session.openWithCallback(self.subsChooserCB, SubsProChooser, self.infobar.subsProSettings, self.subdir, self.embeddedSupport, False, True)
        elif mode == 'search':
            self.searchSubs()
        elif mode == 'settings':
            self.session.openWithCallback(self.subsSetupCB, SubsProSetupMainMisc, self.infobar.subsProSettings)
        elif mode == 'settings_embedded':
            self.session.openWithCallback(self.subsSetupEmbeddedCB, SubsProSetupEmbedded, self.infobar.subsProSettings.embedded)
...
### End EDit
    ...
    def searchSubs(self):
        ...
        def paramsDialogCB(callback=None):
            if callback:
                self.session.openWithCallback(checkDownloadedSubsSelection, SubsProSearch, seeker, subsProSettings.search, sPath, titleList, resetSearchParams=False)
        ...
        def showProvidersErrorCB(callback):
            ...
            if subsProSettings.search.openParamsDialogOnSearch.value:
                self.session.openWithCallback(paramsDialogCB, SubsProSearchParamsMenu, seeker, subsProSettings.search, titleList, enabledList=False)
            else:
                self.session.openWithCallback(checkDownloadedSubsSelection, SubsProSearch, seeker, subsProSettings.search, sPath, titleList)
...
class SubsProSetupExternal(BaseProMenuScreen):
    ...
    def __init__(self, session, externalSettings):
        BaseProMenuScreen.__init__(self, session, _("External Subtitles settings"))
        self.externalSettings = externalSettings
...
class SubsProSetupMainMisc(BaseProMenuScreen):
    ...
    def buildMenu(self):
        configList = []
        configList.append(getConfigListEntry(_("Pause video on opening subtitles menu"), self.subsProSettings.pauseVideoOnSubtitlesMenu))
        configList.append(getConfigListEntry("-" * 200, ConfigNothing()))
        configList.extend(SubsProSetupExternal.getConfigList(self.subsProSettings.external))
        configList.append(getConfigListEntry(_("Encoding"), self.subsProSettings.encodingsGroup))
        configList.append(getConfigListEntry(_("Settings Backup Path"), self.subsProSettings.settingsBackupPath))
        configList.append(getConfigListEntry(_("Settings TMDB data Path"), self.subsProSettings.settingsTMDBDataPath))
        configList.append(getConfigListEntry(_("Theme"), self.subsProSettings.theme))
    ...
    def keySave(self):
        ...
        configfile.save()
        if changedTheme:
            self._pendingThemeCloseArgs = (True, changedEncodingGroup, changedShadowType)
    ...
    def _applyThemeChangesNowCB(self, answer=False):
        closeArgs = getattr(self, "_pendingThemeCloseArgs", (True, False, False))
        if answer:
    ...
    def keyOk(self):
        ...
        if current == self.subsProSettings.settingsBackupPath or currentLabel == _("Settings Backup Path") or currentLabel == "Settings Backup Path":
            self.session.openWithCallback(
                lambda path: self.setSettingsPath(path, self.subsProSettings.settingsBackupPath),
                SubsProFolderSelect,
            ...
            self.session.openWithCallback(
                lambda path: self.setSettingsPath(path, self.subsProSettings.settingsTMDBDataPath),
                SubsProFolderSelect,
...
class SubsProSetupEmbedded(BaseProMenuScreen):
    ...
    def __init__(self, session, embeddedSettings):
        BaseProMenuScreen.__init__(self, session, _("Embedded subtitles settings"))
...
class SubsProSetupGeneral(BaseProMenuScreen):
    ...
    def setsettingsTMDBDataPath(self, settingsTMDBDataPath=None):
        settingsTMDBDataPath = self._normalizeSelectedPath(settingsTMDBDataPath)
...
class SubsProFolderSelect(Screen):
    if isFullHD:
        skin = load_subskin("SubsProFolderSelect_fhd")
    else:
        skin = load_subskin("SubsProFolderSelect_hd")

    def __init__(self, session, title=None, currentPath=None):
        _set_subssupportpro_screen_skin(self, "SubsProFolderSelect")
        Screen.__init__(self, session)
        self.session = session
        ...
        self["help_text"] = Label(_("OK: Open folder  |  Green: Select current folder  |  CH+: First  |  CH-: Last"))
        ...
        try:
            if isFullHD():
                ...
                self["file_list"].l.setFont(0, gFont("Regular", 21))
                self["file_list"].l.setItemHeight(37)
        except Exception:
            pass
    ...
    def updatePathInfo(self):
        try:
            ...
            self["current_path"].setText((_("Current folder") + ": " + self._finishPath(currentDir)))
        except Exception:
            pass
...
class SubsProChooserMenuList(MenuList):
    def __init__(self, embeddedAvailable=False, searchSupport=False, historySupport=False):
        ...
        if searchSupport:
            ...
            menulist.append(res)
        if historySupport:
            # Yellow button: downloaded subtitles.
            res = [('downloaded')]
            ...
            menulist.append(res)
        if searchSupport:
            # Blue button: web subtitles search.
            res = [('search')]
...
class TMDBScraperProScreen(Screen):
    ...
    def __init__(self, session, eventList, settings):
        _set_subssupportpro_screen_skin(self, "TMDBScraperProScreen")
        Screen.__init__(self, session)
        ...
        # /etc/enigma2/subssupportpro and create it if needed.
        ...
        self.tmpPosterPath = "/var/volatile/tmp/subssupportpro_tmdb/"
        ...
        # Keep UI updates in the main Enigma2 thread: the worker thread only
        # changes imageDownloadRunning; this timer hides the message safely.
        self.imageDownloadRunning = False
        self.imageDownloadMessageActive = False
        self.imageDownloadMessageTimer = eTimer()
        self.imageDownloadMessageTimer.callback.append(self.hideImageDownloadMessage)
        self.imageDownloadCheckTimer = eTimer()
        self.imageDownloadCheckTimer.callback.append(self.checkImageDownloadFinished)
    ...
    def updateMovieList(self):
        ...
        for idx, movie in enumerate(self.movies):
            ...
            # Load poster image
            poster_path = os.path.join(self.tmpPosterPath, 'poster_{}.jpg'.format(idx))
            poster_pixmap = None
            ...
            # Create list entry
            listItems.append((poster_pixmap, _e2_list_text(title), _e2_list_text(formatted_date), _e2_list_text(overview)))
        ...
        if self["list"].getCurrent():
            ...
            if 0 <= idx < len(self.movies):
                ...
                if search_title:
                    ...
                    _show_screen_error(self, _("Could not determine search title."))
    ...
    def startBackgroundTasks(self, selected_movie, filepath, title, year):
        ...
        images_folder = os.path.join(self.imagesPath, '{}_{}'.format(title, year))
        ...
        if not details_exist or not images_exist:
            _show_screen_error(self, _("Starting background tasks for details scraping and image download..."))
            ...
            if 0 <= idx < len(self.movies):
                ...
                if os.path.exists(filepath):
                    ...
                    try:
                        ...
                        _show_screen_error(self, _("Error loading details: %s") % str(e))
                else:
                    ...
                    if 'url' in selected_movie:
                        try:
                            ...
                            if details:
                                ...
                                _show_screen_error(self, _("Could not retrieve details for this movie."))
                        except Exception as e:
                            _show_screen_error(self, _("Error retrieving details: %s") % str(e))
                    else:
                        _show_screen_error(self, _("No URL available for this movie."))
                ...
                if os.path.exists(filepath):
                    ...
                    try:
                        ...
                        if release_year != 'unknown_year':
                            ...
                            self.startImageDownloadWithStatus(details, title, release_year)
                        else:
                            _show_screen_error(self, _("Cannot determine year for image folder."))
                    except Exception as e:
                        _show_screen_error(self, _("Error loading details: %s") % str(e))
                else:
                    _show_screen_error(self, _("No details available for this movie. Please view details first."))
            ...
            if details.get('cast'):
                ...
                for i, actor in enumerate(details['cast'][:4]):
                    if actor.get('profile_url'):
                        cast_path = os.path.join(cast_folder, 'cast_{}.jpg'.format(i+1))
                        self.downloadImage(actor['profile_url'], cast_path)
            
            print('Background image download completed for {}_{}'.format(title, release_year))
    ...
    def displayDetails(self, details, filepath):
        ...
        try:
            ...
            pass
        try:
            self.session.openWithCallback(self.movieDetailsClosed, MovieDetailsProScreen, details, images_folder)
        except Exception:
            try:
                self.show()
            except Exception:
...
class MovieDetailsProScreen(Screen):
    ...
    if skin:
        try:
            skin = re.sub(r'zPosition="[^"]+"', 'zPosition="30"', skin, count=1)
        except Exception:
            pass

    def __init__(self, session, details, images_folder):
        _set_subssupportpro_screen_skin(self, "MovieDetailsProScreen", force_zposition=30)
        Screen.__init__(self, session)
    ...
    def _set_label_text(self, widget_name, value):
        try:
            ...
            try:
                print('Error setting MovieDetailsProScreen label {}: {}'.format(widget_name, e))
            except Exception:
                pass
            try:
        ...
        if isinstance(values, (list, tuple)):
            for value in values:
                ...
                try:
                    ...
                    try:
                        out.append(str(value).decode('utf-8', 'ignore') if six.PY2 else str(value))
                    except Exception:
                        pass
        return u", ".join(out)
    ...
    def loadData(self):
        ...
        self._set_label_text("tagline", tagline if tagline else _("No tagline available"))
        ...
        self._set_label_text("runtime", _("Runtime: ") + six.text_type(runtime_text))

        # Adult content
        adult = self.details.get('adult', False)
        self._set_label_text("adult", _("Adult Content: ") + (_("Yes") if adult else _("No")))

        # Genres
        genres = self.details.get('genres', [])
        genres_text = self._join_text_list(genres) if genres else _("No genres available")
        self._set_label_text("genres", _("Genres: ") + six.text_type(genres_text))
        ...
        if os.path.exists(cast_folder):
            ...
            for i, cast_file in enumerate(cast_files[:4]):  # Load up to 4 cast images
                self.loadImage('cast{}'.format(i+1), os.path.join("cast", cast_file))
...
class E2SubsSeeker(SubsSeeker):
    def __init__(self, session, searchSettings, debug=False):
        ...
        class SubsProSearchSettingsProvider(E2SettingsProvider):
        ...
        SubsSeeker.__init__(self, download_path, tmp_path,
                            ...
                            settings_provider_cls=SubsProSearchSettingsProvider,
...
class SubsProChooser(Screen):
    if isFullHD:
        skin = load_subskin("SubsProChooser_fhd")
    else:
        skin = load_subskin("SubsProChooser_hd")
    ...
    def __init__(self, session, subsProSettings, subdir=None, embeddedSupport=False, searchSupport=False, historySupport=False, titleList=None):
        _set_subssupportpro_screen_skin(self, "SubsProChooser")
        Screen.__init__(self, session)
        self.session = session
        self.subsProSettings = subsProSettings
        defaultDir = subdir
        ...
        self["menu_list"] = SubsProChooserMenuList(self.embeddedList, searchSupport, historySupport)
    ...
    def embeddedSubsSelection(self):
        if self.embeddedList:
            self.session.openWithCallback(self.checkEmbeddedSubsSelection, SubsProEmbeddedSelection)

    # Add the tmdbScraperSelection method to the SubsProChooser class:
    def tmdbScraperSelection(self):
        ...
        self.session.openWithCallback(self.tmdbScraperCallback, TMDBScraperProScreen, eventList, settings)
    ...
    def webSubsSelection(self, searchTitles=None):
        ...
        def paramsDialogCB(callback=None):
            if callback:
                self.session.openWithCallback(checkDownloadedSubsSelection, SubsProSearch, seeker, subsProSettings.search, self.videoPath, actualSearchTitles, resetSearchParams=False)

        def showProvidersErrorCB(callback):
            if not callback:
                subsProSettings.search.showProvidersErrorMessage.value = False
            if subsProSettings.search.openParamsDialogOnSearch.value:
                self.session.openWithCallback(paramsDialogCB, SubsProSearchParamsMenu, seeker, subsProSettings.search, actualSearchTitles, enabledList=False)
            else:
                self.session.openWithCallback(checkDownloadedSubsSelection, SubsProSearch, seeker, subsProSettings.search, self.videoPath, actualSearchTitles)
    ...
    def downloadedSubsSelectionCB(self, subtitles, downloadedSubtitle=None):
        fpath = os.path.join(self.subsProSettings.search.downloadHistory.path.value, 'hsubtitles.json')
        try:
    ...
    def downloadedSubsSelection(self):
        ...
        try:
            ...
            subtitles = []
        self.session.openWithCallback(self.downloadedSubsSelectionCB, SubsProDownloadedSelection, subtitles, self.subsProSettings.search.downloadHistory)


class SubsProDownloadedSelection(Screen):
    class InfoProScreen(Screen):
        ...
        def __init__(self, session, subtitle):
            _set_subssupportpro_screen_skin(self, "InfoProScreen")
            Screen.__init__(self, session)
            self["path"] = StaticText(_(toString(subtitle['fpath'])))

    if isFullHD:
        skin = load_subskin("SubsProDownloadedSelection_fhd")
    else:
        skin = load_subskin("SubsProDownloadedSelection_hd")

    def __init__(self, session, subtitles, historySettings, marked=None):
        _set_subssupportpro_screen_skin(self, "SubsProDownloadedSelection")
        Screen.__init__(self, session)
    ...
    def updateSubsList(self):
        ...
        for sub in self.subtitles[:]:
            ...
            if not os.path.isfile(fpath):
                ...
                continue
            countryPng = _subpro_country_png(sub.get('country', 'missing'), imgDict)
            ...
            subtitleListGUI.append((countryPng, _e2_list_text(name), _e2_list_text(sub.get('provider', '')), _e2_list_text(date), color),)
        imgDict = None
            ...
            if doRemove:
                if self.historySettings.removeAction.value == 'file':
                    try:
                        ...
                        print("[SubsProDownloadedSelection] cannot remove - %s" % (str(e)))
    ...
    def showInfo(self):
        if self["subtitles"].count() > 0:
            ...
            self.__infoScreen = self.session.instantiateDialog(self.InfoProScreen, subtitle)
...
class SubsDownloadedSubtitlesMenu(BaseProMenuScreen):
    def __init__(self, session, historySettings):
        BaseProMenuScreen.__init__(self, session, _("Downloaded Subtitles - Settings"))
        self.historySettings = historySettings
...
class SubsProEmbeddedSelection(Screen):
    if isFullHD:
        skin = load_subskin("SubsProEmbeddedSelection_fhd")
    else:
        skin = load_subskin("SubsProEmbeddedSelection_hd")

    def __init__(self, session):
        _set_subssupportpro_screen_skin(self, "SubsProEmbeddedSelection")
        Screen.__init__(self, session)
...
class SubsProSearchProcess(object):
    processes = []
    process_path = os.path.join(os.path.dirname(__file__), 'searchsubs.py')

    def __init__(self):
        self.log = SimpleLogger('SubsProSearchProcess', SimpleLogger.LOG_INFO)
        self.toRead = None
        self.pPayload = None
        self.data = ""
    ...
    def _getSuggestions(self, queryString):
        ...
        def fetch():
            response = requests.get(
                ...
                headers={
                    "Api-Key": apiKey,
                    "User-Agent": "SubsSupportPro/%s" % __version__,
...
class BaseSuggestionsProListScreen(Screen):
    def __init__(self, session, title, configTextWithSuggestions, positionX, titleColor):
        desktopSize = getDesktopSize()
        titleText = "Suggestions" if titleColor == "green" else "History"
        ...
        if isFullHD():
            ...
            if titleColor == "green":
                ...
                positionX = 50
            positionX = min(max(0, positionX), max(0, int(desktopSize[0]) - windowSize[0] - 10))
            positionY = 10
            self.skin = """
                <screen position="%d,%d" size="800,260" flags="wfNoBorder" backgroundColor="#ff000000" zPosition="10">
                    <eLabel position="0,0" size="800,260" backgroundColor="#061425" zPosition="-5" cornerRadius="20" />
                    <eLabel position="12,12" size="776,236" backgroundColor="#0a1b2d" zPosition="-4" cornerRadius="14" />

                    <widget source="suggestionstitle" render="Label" position="30,18" size="740,34" font="Regular;24" halign="center" valign="center" foregroundColor="%s" backgroundColor="#061425" />

                    <widget source="suggestionslist" render="Listbox" position="30,60" size="740,180" scrollbarMode="showAlways" transparent="1" scrollbarWidth="10" itemHeight="30" itemCornerRadius="8" itemCornerRadiusSelected="8" backgroundColorSelected="#45818e" backgroundColor="#061425">
                        <convert type="TemplatedMultiContent">
                            {"templates":
                                {"default": (30, [
                                    MultiContentEntryText(pos = (12, 0), size = (700, 30), font = 0, flags = RT_HALIGN_LEFT|RT_VALIGN_CENTER, text = 0, color=0x00EAF3FF),
                                ], True, "showOnDemand"),
                                "notselected": (30, [
                                    MultiContentEntryText(pos = (12, 0), size = (700, 30), font = 0, flags = RT_HALIGN_LEFT|RT_VALIGN_CENTER, text = 0, color=0x00D6E4F7),
                                ], False, "showOnDemand")
                                },
            ...
            """ % (positionX, positionY, titleForeground)
        else:
            windowSize = (680, 230)
            positionX = min(positionX, max(0, int(desktopSize[0]) - windowSize[0] - 20))
            positionY = 10
            self.skin = """
                <screen position="%d,%d" size="680,230" flags="wfNoBorder" backgroundColor="#ff000000" zPosition="10">
                    <eLabel position="0,0" size="680,230" backgroundColor="#061425" zPosition="-5" cornerRadius="14" />
                    <eLabel position="12,12" size="656,206" backgroundColor="#0a1b2d" zPosition="-4" cornerRadius="10" />

                    <widget source="suggestionstitle" render="Label" position="30,20" size="620,30" font="Regular;20" halign="center" valign="center" foregroundColor="#59d8ff" backgroundColor="#061425" />

                    <widget source="suggestionslist" render="Listbox" position="30,60" size="620,160" scrollbarMode="showAlways" transparent="1" scrollbarWidth="8" itemHeight="40" itemCornerRadius="8" itemCornerRadiusSelected="8" backgroundColorSelected="#45818e" backgroundColor="#061425">
                        <convert type="TemplatedMultiContent">
                            {"templates":
                                {"default": (40, [
                                    MultiContentEntryText(pos = (10, 0), size = (590, 40), font = 0, flags = RT_HALIGN_LEFT|RT_VALIGN_CENTER, text = 0, color=0x00EAF3FF),
                                ], True, "showOnDemand"),
                                "notselected": (40, [
                                    MultiContentEntryText(pos = (10, 0), size = (590, 40), font = 0, flags = RT_HALIGN_LEFT|RT_VALIGN_CENTER, text = 0, color=0x00D6E4F7),
                                ], False, "showOnDemand")
                                },
            ...
            """ % (positionX, positionY)
        self.skin = _apply_subssupportpro_version(_apply_subssupportpro_theme(self.skin))
        Screen.__init__(self, session)
        self.list = []
...
class SuggestionsProListScreen(BaseSuggestionsProListScreen):
    def __init__(self, session, configTextWithSuggestions):
        ...
        BaseSuggestionsProListScreen.__init__(self, session, title, configTextWithSuggestions, positionX, 'green')
...
class HistoryProListScreen(BaseSuggestionsProListScreen):
    def __init__(self, session, configTextWithSuggestions):
        ...
        BaseSuggestionsProListScreen.__init__(self, session, title, configTextWithSuggestions, positionX, 'red')
    ...
    def onSelect(self, session):
        ...
        if session is not None:
            if self.suggestionsWindow is None:
                self.suggestionsWindow = session.instantiateDialog(SuggestionsProListScreen, self)
            self.suggestionsWindow.deactivate()
            self.suggestionsWindow.show()
            if self.historyWindow is None:
                self.historyWindow = session.instantiateDialog(HistoryProListScreen, self)
...
class SubsProSearchDownloadOptions(Screen, ConfigListScreen):
    if isFullHD:
        skin = load_subskin("SubsProSearchDownloadOptions_fhd")
    else:
        skin = load_subskin("SubsProSearchDownloadOptions_hd")

    def __init__(self, session, subtitle, saveAs, saveTo, addLang, dPath, vPath=None):
        _set_subssupportpro_screen_skin(self, "SubsProSearchDownloadOptions")
        Screen.__init__(self, session)
        saveAsOptions = []
...
class SubsProSearchContextMenu(Screen):
    if isFullHD:
        skin = load_subskin("SubsProSearchContextMenu_fhd")
    else:
        skin = load_subskin("SubsProSearchContextMenu_hd")

    def __init__(self, session):
        _set_subssupportpro_screen_skin(self, "SubsProSearchContextMenu")
        Screen.__init__(self, session)
        self.options = []
...
class SubsSupportProLogScreen(Screen):
    if isFullHD:
        skin = load_subskin("SubsSupportProLogScreen_fhd")
    else:
        skin = load_subskin("SubsSupportProLogScreen_hd")
    
    def __init__(self, session):
        _set_subssupportpro_screen_skin(self, "SubsSupportProLogScreen")
        Screen.__init__(self, session)
        self.setTitle(_("SubsSupportPro Log"))
    ...
    def loadLog(self):
        log_content = ""
        log_path = "/var/volatile/tmp/subssupportpro.log"
        try:
...
class SubsProSearch(Screen):
    if isFullHD:
        skin = load_subskin("SubsProSearch_fhd")
    else:
        skin = load_subskin("SubsProSearch_hd")

    def __init__(self, session, seeker, searchSettings, filepath=None, searchTitles=None, resetSearchParams=True, standAlone=False):
        _set_subssupportpro_screen_skin(self, "SubsProSearch")
        Screen.__init__(self, session)
        self.searchSettings = searchSettings
        ...
        self.__contextMenu = self.session.instantiateDialog(SubsProSearchContextMenu)
    ...
    def showLogScreen(self):
        self.session.open(SubsSupportProLogScreen)
    ...
    def updateSearchInfoList(self):
        ...
        searchInfoList.append((_e2_list_text(_("Title") + ":"), _e2_list_text(self.searchTitle.value)))
        searchInfoList.append((_e2_list_text(_("Type") + ":"), _e2_list_text(self.searchType.getText())))
        if self.searchType.value == "movie":
            searchInfoList.append((_e2_list_text(_("Year") + ":"), _e2_list_text(year)))
        else:
            searchInfoList.append((_e2_list_text(_("Season") + ":"), _e2_list_text(season)))
            searchInfoList.append((_e2_list_text(_("Episode") + ":"), _e2_list_text(episode)))
        searchInfoList.append((_e2_list_text(_("Provider") + ":"), _e2_list_text(self.searchProvider.getText())))
        searchInfoList.append((_e2_list_text(_("Preferred languages") + ":"), _e2_list_text(languages)))
        searchInfoList.append((_e2_list_text(_("Use File path") + ":"), _e2_list_text(useFilePathStr)))
    ...
    def updateSubsList(self):
        ...
        for sub in self.subtitlesList:
            ...
            subtitleListGUI.append((
                countryPng,
                _e2_list_text(sub.get('language_name', '')),
    ...
    def searchSubs(self):
        def searchSubsUpdate(args):
            ...
            if not hasattr(self, '_SubsProSearch__finished'):
                self.__finished = {}
            if status:
        ...
        SubsProSearchProcess().start(params, callbacks)
    ...
    def stopSearchSubs(self):
        for p in SubsProSearchProcess.processes:
            p.stop()
        print(len(SubsProSearchProcess.processes), 'processes still running')

    def searchSubsSuccess(self, subtitles):
        print('[SubsProSearch] search success')
        self.message.hide()
    ...
    def searchSubsError(self, error):
        print('[SubsProSearch] search error', str(error))
        try:
    ...
    def downloadSubsSuccess(self, subFile):
        print('[SubsProSearch] download success %s' % toString(subFile))
        dsubtitle = {
        ...
        if self.searchSettings.downloadHistory.enabled.value:
            ...
            try:
                ...
                print('[SubsProSearch] cannot load download history:', e)
                subtitles = []
            ...
            if len(subtitles) >= limit:
                print('[SubsProSearch] download history limit reached!, removing oldest entries')
                del subtitles[-(len(subtitles) - limit):]
            subtitles.insert(0, dsubtitle)
            try:
                json.dump(subtitles, open(fpath, 'w'))
            except Exception as e:
                print('[SubsProSearch] cannot save download history:', e)
    ...
    def downloadSubsError(self, e):
        print('[SubsProSearch] download error', str(e))
        self.__downloading = False
        del self.__downloadingSubtitle
        self.updateBottomMenu()
        self.updateActionMaps()
    ...
    def cancelDownloadSubs(self):
        print('[SubsProSearch] download cancelled')
        self.__downloading = False
        del self.__downloadingSubtitle
        self.stopSearchSubs()
        self.updateBottomMenu()
    ...
    def contextMenuOk(self):
        ...
        if answer == 'd_more' or answer == 'do_more':
            ...
            vPath = self.filepath
            self.session.openWithCallback(downloadMoreCB, SubsProSearchDownloadOptions,
...
### End EDit
    ...
    def updatePoster(self):
        ...
        try:
            ...
            print('[SubsProSearch] Looking for poster for: {} ({})'.format(title, year))
            ...
            if title:
                ...
                if not os.path.exists(images_dir):
                    print('[SubsProSearch] Images directory does not exist: {}'.format(images_dir))
                ...
                if matching_folder:
                    ...
                    print('[SubsProSearch] Trying poster path: {}'.format(poster_path))
                    ...
                    if os.path.exists(poster_path):
                        ...
                        try:
                            ...
                            if pixmap:
                                ...
                                self["poster"].show()
                                print('[SubsProSearch] Poster loaded successfully: {}'.format(poster_path))
                                ...
                                print('[SubsProSearch] Failed to load pixmap: {}'.format(poster_path))
                        except Exception as e:
                            print('[SubsProSearch] Error loading poster: {}'.format(e))
                    else:
                        print('[SubsProSearch] Poster file does not exist: {}'.format(poster_path))
                else:
                    print('[SubsProSearch] No matching folder found for: {}'.format(title))
                    # List available folders for debugging
                    available_folders = [f for f in os.listdir(images_dir) if os.path.isdir(os.path.join(images_dir, f))]
                    print('[SubsProSearch] Available folders: {}'.format(available_folders))
            ...
            print('[SubsProSearch] Error in updatePoster: {}'.format(e))
            import traceback
            traceback.print_exc()
            self.showDefaultPoster()
            self.startPosterRefreshTimer()
    ...
    def generatePossibleFolderNames(self, title, year):
        ...
        if year:
            ...
            possible_folders.append('{}_{}'.format(clean_title.replace(' ', '_'), year))
    ...
    def startPosterRefreshTimer(self):
        ...
        if self.posterRefreshCount < self.maxPosterRefreshAttempts:
            ...
            print('[SubsProSearch] Poster refresh scheduled (attempt {}/{})'.format(self.posterRefreshCount, self.maxPosterRefreshAttempts))
        else:
            print("[SubsProSearch] Max poster refresh attempts reached")
    
    def refreshPosterPeriodically(self):
        """Periodically check if the poster has been downloaded"""
        print("[SubsProSearch] Refreshing poster check...")
        self.updatePoster()
    
    def showDefaultPoster(self):
        """Show default poster when no specific poster is found"""
        try:
            default_poster = "/usr/lib/enigma2/python/Plugins/Extensions/SubsSupportPro/img/default_poster.jpg"
            ...
            if os.path.exists(default_poster):
                ...
                self["poster"].show()
                print("[SubsProSearch] Default poster loaded")
            else:
                print("[SubsProSearch] Default poster not found, hiding poster widget")
                self["poster"].hide()
        except Exception as e:
            print('[SubsProSearch] Error loading default poster: {}'.format(e))
            self["poster"].hide()
        ...
        def updateSearchParamsCB(callback=None):
            if callback:
                ...
                if not self.searchSettings.manualSearch.value:
                    self.searchSubs()
        self.session.openWithCallback(updateSearchParamsCB, SubsProSearchParamsMenu, self.seeker, self.searchSettings, self.searchTitles, False)
    ...
    def openDownloadHistory(self):
        def openDownloadHistoryCB(subtitles, subtitle=None):
            if len(subtitles) > 0:
                ...
                try:
                    ...
                    print('[SubsProSearch] save download history:', e)
        ...
        try:
            ...
            print('[SubsProSearch] cannot load download history:', e)
            subtitles = []
        self.session.openWithCallback(openDownloadHistoryCB, SubsProDownloadedSelection,
    ...
    def openSettings(self):
        ...
        self.session.openWithCallback(openSettingsCB, SubsProSearchSettings, self.searchSettings, self.seeker, self.isLocalFilepath)



class SubsProSearchSettings(Screen, ConfigListScreen):
    ...
    if isFullHD:
        skin = load_subskin("SubsProSearchSettings_fhd")
    else:
        skin = load_subskin("SubsProSearchSettings_hd")

    FOCUS_CONFIG, FOCUS_PROVIDERS = range(2)

    def __init__(self, session, searchSettings, seeker, isLocalFilepath=True):
        _set_subssupportpro_screen_skin(self, "SubsProSearchSettings")
        Screen.__init__(self, session)
    ...
    def _getProviderLogoPixmap(self, provider):
        ...
        if '.seekers.' in module_name:
            try:
                candidates.append(module_name.split('.seekers.', 1)[1].split('.', 1)[0])
            except Exception:
                pass
        ...
        for attr in ('id', 'provider_name', 'name'):
            ...
            if value:
                ...
                if value:
                    ...
                    candidates.append(value.replace(' ', '').replace('-', '').replace('_', ''))
    ...
    def keyOk(self):
        if self.focus == self.FOCUS_PROVIDERS:
            ...
            if current == self.searchSettings.downloadPath:
                ...
                self.session.openWithCallback(self.setLanguage, SubsProLanguageSelection, current.value)
    ...
    def openProviderSettings(self, provider):
        self.session.openWithCallback(self.openProviderSettingsCB, SubsProSearchProviderMenu, provider)
...
class SubsProSearchParamsMenu(Screen, ConfigListScreen):
    LIST_CONFIG = 0
    LIST_SUGGESTIONS = 1
    LIST_HISTORY = 2
    ...
    def __init__(self, session, seeker, searchSettings, titleList=None, resetSearchParams=True, enabledList=True, windowTitle=None):
        ...
        if isFullHD():
            self.skin = """
                <screen position="450,400" size="1020,630" flags="wfNoBorder" backgroundColor="#ff000000" zPosition="10">
                    <eLabel position="0,0" size="1020,630" backgroundColor="#061425" zPosition="-5" cornerRadius="20" />
                    <eLabel position="20,20" size="980,590" backgroundColor="#0a1b2d" zPosition="-4" cornerRadius="14" />
                    <eLabel position="30,30" size="960,68" backgroundColor="#8e5245" zPosition="-3" cornerRadius="20" borderWidth="1" borderColor="#27408b" />
                    <eLabel position="30,108" size="960,2" backgroundColor="#5d37d9" zPosition="-3" />
                    <eLabel name="" position="40,40" size="940,48" zPosition="5" text="Update Search Parameters" backgroundColor="#8e5245" font="Regular;34" valign="center" halign="center" foregroundColor="#eaf3ff" />
                    <widget source="sourceTitleInfo" render="Label" position="50,125" size="920,40" halign="center" valign="center" font="Regular;28" foregroundColor="#59d8ff" backgroundColor="#061425" />
                    <widget source="sourceTitle" render="Label" position="50,175" size="920,72" halign="center" valign="center" font="Regular;28" foregroundColor="#eaf3ff" backgroundColor="#061425" />
                    <eLabel position="50,265" size="920,2" backgroundColor="#5d37d9" />
                    <widget name="config" position="50,290" size="920,240" font="Regular;28" itemHeight="40" scrollbarMode="showNever" backgroundColorSelected="#45818e" backgroundColor="#061425" itemCornerRadius="10" itemCornerRadiusSelected="10" />
                    <widget source="key_blue" render="Label" position="104,548" size="250,50" halign="left" valign="center" zPosition="2" font="Regular;28" foregroundColor="white" backgroundColor="#1f58d6" transparent="1" />
                    <eLabel name="" position="50,548" size="52,52" cornerRadius="26" backgroundColor="#999999" zPosition="-1" />
                    <eLabel name="" position="50,548" size="50,50" cornerRadius="25" backgroundColor="#1f58d6" zPosition="1" />
                </screen>
            """
        else:
            self.skin = """
...
class SubsProSearchProviderMenu(BaseProMenuScreen):
    def __init__(self, session, provider):
        ...
        self.backup_path = "/etc/enigma2/subssupportpro"        
        if not os.path.exists(self.backup_path):
            os.makedirs(self.backup_path, mode=0o755)        
        self.backup_file = os.path.join(self.backup_path, '{}_credentials.json'.format(provider.id))
        ...
        # Initialize Menu and Info components using safe names (lbl_) to avoid E2 conflicts
        self["lbl_menu"] = Label("")
        self["lbl_info"] = Label("")
        self["lbl_menu_circle"] = Label("")
        self["lbl_menu_shadow"] = Label("")
        self["lbl_info_circle"] = Label("")
        self["lbl_info_shadow"] = Label("")
        self.hideYellowKey()
        ...
        # Setup logical action mapping if credentials exist
    ...
    def _applyMenuButtonsState(self):
            """Safely applies text and visibility to buttons after E2 UI initialization"""
            if self._has_credentials():
                # Added hardware button names so the user knows what to press
                self["lbl_menu"].setText("MENU: " + _("Backup"))
                self["lbl_info"].setText("INFO: " + _("Restore"))
                for widget in ("lbl_menu", "lbl_menu_circle", "lbl_menu_shadow", "lbl_info", "lbl_info_circle", "lbl_info_shadow"):
                    if widget in self:
                        self[widget].show()
            else:
